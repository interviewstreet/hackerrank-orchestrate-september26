...
def checkpoint_path(samples: bool) -> Path:
    """Separate checkpoints per scope, so a sample run and a full run cannot
    contaminate each other's --resume state."""
    name = "checkpoint_samples.jsonl" if samples else "checkpoint.jsonl"
    return CODE_DIR / ".cache" / name
...
def build_agent(settings: Settings, tracker: TokenTracker, loader: DatasetLoader,
                converter: ExchangeConverter, use_rag: bool, logger):
    """Assemble the orchestrator and its tools."""
...
def main() -> None:
    ...
    if not args.no_llm:
        ...
        agent = build_agent(
            settings, tracker, loader, converter, not args.no_rag, logger
        )

    checkpoint = checkpoint_path(args.samples)
    ...
    # Emit in the dataset's own order, never completion order.
    rows = [results[i] for i in sorted(results)]

    output_paths = (
        (args.output,) if args.output else (
            settings.output_paths if settings else
            (REPO_ROOT / "output.csv", REPO_ROOT / "dataset" / "output.csv")
        )
    )
